...
/**
 * Gate for the coach-facing Team Analysis tools. Any subscriber with premium
 * access can run team video reviews; non-subscribers see an upsell.
 */
export const PremiumCoachGate: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const { profile } = useAuth();
  if (profile?.has_premium_access) return <>{children}</>;
  return (
    <View style={s.container}>
      <Text style={s.icon}>🎯</Text>
      <Text style={s.title}>Coach tools are premium</Text>
      <Text style={s.body}>
        Team Analysis lets you review your roster&apos;s runs with AI coaching cues. Unlock it with a
        Rodeo Apps Premium subscription.
      </Text>
